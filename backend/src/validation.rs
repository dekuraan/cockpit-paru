...
pub fn validate_aur_package_name(name: &str) -> Result<()> {
    if name.is_empty() {
        anyhow::bail!("AUR package name cannot be empty");
    }
    if name.len() > 256 {
        anyhow::bail!("AUR package name too long (max 256)");
    }
    if name.chars().any(|c| c.is_control()) {
        anyhow::bail!("AUR package name contains invalid control characters");
    }
    if name.contains("..") || name.contains('/') || name.contains('\\') {
        anyhow::bail!("AUR package name contains path traversal characters");
    }
    Ok(())
}

pub fn validate_aur_search_query(query: &str) -> Result<()> {
    if query.len() < 2 {
        anyhow::bail!("AUR search query must be at least 2 characters");
    }
    if query.len() > 256 {
        anyhow::bail!("AUR search query too long (max 256)");
    }
    if query.chars().any(|c| c.is_control()) {
        anyhow::bail!("AUR search query contains invalid characters");
    }
    Ok(())
}
