...
use cockpit_paru_backend::handlers::{
    add_ignored, aur_check_updates, aur_get_pkgbuild, aur_info, aur_install, aur_search,
    aur_upgrade, check_updates, clean_cache, downgrade_package, fetch_mirror_status, fetch_news,
...
use cockpit_paru_backend::models::MirrorEntry;
use cockpit_paru_backend::validation::{
    validate_aur_package_name, validate_aur_search_query, validate_depth, validate_direction,
    validate_json_payload_size, validate_keep_versions, validate_mirror_timeout,
    validate_mirror_url, validate_package_name, validate_pagination, validate_search_query,
};

fn print_usage() {
    eprintln!("Usage: cockpit-paru-backend <command> [args]");
    eprintln!();
    eprintln!("Commands:");
    ...
    eprintln!();
    eprintln!("AUR Commands:");
    eprintln!("  aur-search QUERY [offset] [limit] [sort_by] [sort_dir]");
    eprintln!("                         Search AUR packages");
    eprintln!("                         sort_by: name|votes|popularity");
    eprintln!("                         sort_dir: asc|desc");
    eprintln!("  aur-info NAME          Get detailed AUR package info");
    eprintln!("  aur-check-updates      Check for AUR package updates");
    eprintln!("  aur-get-pkgbuild NAME  Get PKGBUILD content for an AUR package");
    eprintln!("  aur-install NAME [timeout]");
    eprintln!("                         Install an AUR package (streaming, requires root)");
    eprintln!("                         timeout: seconds (default: 600)");
    eprintln!("  aur-upgrade [timeout]  Upgrade all AUR packages (streaming, requires root)");
    eprintln!("                         timeout: seconds (default: 600)");
}
...
fn main() {
    ...
    let result = match args[1].as_str() {
        ...
        "aur-search" => {
            if args.len() < 3 {
                eprintln!("Error: aur-search requires a query");
                std::process::exit(1);
            }
            let offset = args.get(3).and_then(|s| s.parse().ok()).unwrap_or(0);
            let limit = args.get(4).and_then(|s| s.parse().ok()).unwrap_or(100);
            let sort_by = args.get(5).map(|s| s.as_str()).filter(|s| !s.is_empty());
            let sort_dir = args.get(6).map(|s| s.as_str()).filter(|s| !s.is_empty());
            validate_aur_search_query(&args[2])
                .and_then(|_| validate_pagination(offset, limit))
                .and_then(|_| aur_search(&args[2], offset, limit, sort_by, sort_dir))
        }
        "aur-info" => {
            if args.len() < 3 {
                eprintln!("Error: aur-info requires a package name");
                std::process::exit(1);
            }
            validate_aur_package_name(&args[2]).and_then(|_| aur_info(&args[2]))
        }
        "aur-check-updates" => aur_check_updates(),
        "aur-get-pkgbuild" => {
            if args.len() < 3 {
                eprintln!("Error: aur-get-pkgbuild requires a package name");
                std::process::exit(1);
            }
            validate_aur_package_name(&args[2]).and_then(|_| aur_get_pkgbuild(&args[2]))
        }
        "aur-install" => {
            if args.len() < 3 {
                eprintln!("Error: aur-install requires a package name");
                std::process::exit(1);
            }
            let timeout = args.get(3).and_then(|s| s.parse().ok());
            validate_aur_package_name(&args[2]).and_then(|_| aur_install(&args[2], timeout))
        }
        "aur-upgrade" => {
            let timeout = args.get(2).and_then(|s| s.parse().ok());
            aur_upgrade(timeout)
