...
const CONFIG_PATH: &str = "/etc/cockpit-paru/config.json";
const TIMER_DROP_IN_DIR: &str = "/etc/systemd/system/cockpit-paru-scheduled.timer.d";
const TIMER_DROP_IN_PATH: &str =
    "/etc/systemd/system/cockpit-paru-scheduled.timer.d/schedule.conf";
...
impl AppConfig {
    ...
    pub fn apply_schedule_to_systemd(&self) -> Result<()> {
        ...
        if schedule.enabled {
            ...
            let output = Command::new("systemctl")
                .args(["enable", "--now", "cockpit-paru-scheduled.timer"])
                .output()
            ...
            let _ = Command::new("systemctl")
                .args(["disable", "--now", "cockpit-paru-scheduled.timer"])
                .output();
