...
import { ARCH_STATUS_URL, LOG_CONTAINER_HEIGHT, MAX_LOG_SIZE_BYTES, NEWS_LOOKBACK_DAYS, STREAMING_TIMEOUT_SECS } from "../constants";
...
export const UpdatesView: React.FC<UpdatesViewProps> = ({ onViewDependencies }) => {
  ...
  // AUR updates state
  const [aurUpdates, setAurUpdates] = useState<AurUpdateInfo[]>([]);
  const [aurUpdatesLoading, setAurUpdatesLoading] = useState(false);
  const [aurUpdatesError, setAurUpdatesError] = useState<string | null>(null);
  const [aurUpgrading, setAurUpgrading] = useState(false);
  const [aurUpgradeLog, setAurUpgradeLog] = useState("");
  const [aurUpgradeSuccess, setAurUpgradeSuccess] = useState<boolean | null>(null);
  const [aurUpgradeError, setAurUpgradeError] = useState<string | null>(null);
  const aurUpgradeCancelRef = useRef<(() => void) | null>(null);
  const aurLogContainerRef = useAutoScrollLog(aurUpgradeLog);
  ...
  const loadAurUpdates = useCallback(async () => {
    setAurUpdatesLoading(true);
    setAurUpdatesError(null);
    try {
      const response = await aurCheckUpdates();
      setAurUpdates(response.updates);
    } catch (ex) {
      setAurUpdatesError(ex instanceof Error ? ex.message : String(ex));
    } finally {
      setAurUpdatesLoading(false);
    }
  }, []);
  ...
  const handleAurUpgrade = () => {
    setAurUpgrading(true);
    setAurUpgradeLog("");
    setAurUpgradeSuccess(null);
    setAurUpgradeError(null);

    const { cancel } = aurUpgrade({
      onData: (data) => setAurUpgradeLog((prev) => prev + data),
      onComplete: () => {
        setAurUpgrading(false);
        setAurUpgradeSuccess(true);
        loadAurUpdates();
      },
      onError: (err) => {
        setAurUpgrading(false);
        setAurUpgradeSuccess(false);
        setAurUpgradeError(err);
      },
      timeout: STREAMING_TIMEOUT_SECS,
    });
    aurUpgradeCancelRef.current = cancel;
  };

  const handleAurUpgradeCancel = () => {
    aurUpgradeCancelRef.current?.();
  };
  ...
  useEffect(() => {
    return () => {
      ...
      if (aurUpgradeCancelRef.current) {
        aurUpgradeCancelRef.current();
      }
    };
  }, []);
    ...
    <>
      ...
      <Card className="pf-v6-u-mt-md">
        <CardBody>
          <Flex justifyContent={{ default: "justifyContentSpaceBetween" }} alignItems={{ default: "alignItemsCenter" }} className="pf-v6-u-mb-md">
            <FlexItem>
              <CardTitle className="pf-v6-u-m-0">
                AUR Updates
                {!aurUpdatesLoading && aurUpdates.length > 0 && (
                  <Label color="orange" className="pf-v6-u-ml-sm">{aurUpdates.length}</Label>
                )}
              </CardTitle>
            </FlexItem>
            <FlexItem>
              <Button
                variant="secondary"
                onClick={loadAurUpdates}
                isDisabled={aurUpdatesLoading || aurUpgrading}
                icon={<SyncAltIcon />}
                className="pf-v6-u-mr-sm"
              >
                Check
              </Button>
              {aurUpdates.length > 0 && !aurUpgrading && (
                <Button
                  variant="primary"
                  onClick={handleAurUpgrade}
                  isDisabled={aurUpdatesLoading}
                >
                  Upgrade AUR ({aurUpdates.length})
                </Button>
              )}
              {aurUpgrading && (
                <Button variant="danger" onClick={handleAurUpgradeCancel}>
                  Cancel
                </Button>
              )}
            </FlexItem>
          ...
          {aurUpdatesLoading && <Spinner size="md" />}
          {aurUpdatesError && (
            <Alert variant="danger" isInline title="Failed to check AUR updates" className="pf-v6-u-mb-md">
              {aurUpdatesError}
            </Alert>
          )}
          {aurUpgradeSuccess === true && (
            <Alert variant="success" isInline title="AUR upgrade completed" className="pf-v6-u-mb-md" />
          )}
          {aurUpgradeSuccess === false && aurUpgradeError && (
            <Alert variant="danger" isInline title="AUR upgrade failed" className="pf-v6-u-mb-md">
              {aurUpgradeError}
            </Alert>
          )}
          ...
          {aurUpgrading && (
            ...
            >
              {aurUpgradeLog || "Starting paru...\n"}
            </div>
          )}

          {!aurUpdatesLoading && !aurUpgrading && aurUpdates.length === 0 && !aurUpdatesError && (
            <Content component={ContentVariants.p}>All AUR packages are up to date.</Content>
          )}

          {!aurUpdatesLoading && !aurUpgrading && aurUpdates.length > 0 && (
            <Table aria-label="AUR updates" variant="compact">
              <Thead>
                <Tr>
                  <Th>Name</Th>
              ...
              <Tbody>
                {aurUpdates.map((update) => (
                  <Tr key={update.name}>
                    <Td dataLabel="Name">{update.name}</Td>
                    <Td dataLabel="Current Version">{update.current_version}</Td>
                    <Td dataLabel="New Version">{update.new_version}</Td>
                    <Td dataLabel="Maintainer">{update.maintainer || <Label color="orange">Orphaned</Label>}</Td>
                    <Td dataLabel="Status">
                      {update.out_of_date != null && (
                        <Label color="orange">Flagged out of date</Label>
                      )}
                    </Td>
                  </Tr>
                ))}
